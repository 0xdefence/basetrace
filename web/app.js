...
function renderFailuresPanel() {
  ...
  document.getElementById('failures').innerHTML = `<table class="table"><thead><tr><th>ID</th><th>Range</th><th>Retries</th><th>Status</th><th>Actions</th></tr></thead><tbody>${rows
    .map(
      (r) => `<tr><td>${r.id}</td><td>${r.start_block ?? '-'} → ${r.end_block ?? '-'}</td><td>${r.retry_count ?? 0}</td><td>${r.status || ''}</td><td class="actions"><button class="secondary fail-retry" data-id="${r.id}">Retry</button><button class="danger fail-resolve" data-id="${r.id}">Resolve</button></td></tr><tr><td></td><td colspan="4" class="muted">${(r.error || '').slice(0, 160)}</td></tr>`
    )
    .join('')}</tbody></table>`;
...
function renderFlowGraph() {
  ...
  const lines = edges.filter((e) => pos[e.src.toLowerCase()] && pos[e.dst.toLowerCase()]).slice(0, 28).map((e) => {
    const s = pos[e.src.toLowerCase()], d = pos[e.dst.toLowerCase()];
    return `<line class="g-link" x1="${s.x}" y1="${s.y}" x2="${d.x}" y2="${d.y}"/>`;
  }).join('');
  const circles = Object.entries(pos).map(([id, p]) => `<circle class="g-node ${id === center ? 'center' : ''}" cx="${p.x}" cy="${p.y}" r="${id === center ? 16 : 11}"/><text class="g-label" x="${p.x + 12}" y="${p.y + 4}">${shortAddr(id)}</text>`).join('');
  svg.innerHTML = `${lines}${circles}`;
  document.getElementById('graphMeta').textContent = `nodes=${nodes.length} edges=${edges.length} center=${shortAddr(center)}`;
}
...
async function refreshAll() {
  ...
  let graph = { nodes: [], edges: [], center: '' };
  if (seed) {
    const g = await jfetch(`/graph/neighbors/${seed}?limit=30`);
...
function bind() {
  ...
  document.getElementById('graphLoadBtn').addEventListener('click', () => runSafe(async () => {
    const a = document.getElementById('graphAddress').value.trim();
    if (!a) return;
    const g = await jfetch(`/graph/neighbors/${a}?limit=30`);
  ...
  document.getElementById('graphLoadKnownBtn').addEventListener('click', () => runSafe(async () => {
    ...
    for (const a of known) {
      try { const g = await jfetch(`/graph/neighbors/${a}?limit=30`); store.set({ graph: { ...g, center: a.toLowerCase() }, activePanel: 'flow' }); document.getElementById('graphAddress').value = a; return; } catch {}
    }
  }));
  ...
  document.body.addEventListener('click', (e) => runSafe(async () => {
    ...
    if (t.matches('.addr-btn')) {
      const address = t.dataset.address; if (!address) return;
